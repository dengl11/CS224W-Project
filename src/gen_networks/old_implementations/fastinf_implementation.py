...
def output_network(E_exp,E_pl,E_simple):
	E_exp_s = sorted(E_exp.items(), key=operator.itemgetter(0), reverse=True)
	E_simple_s = sorted(E_simple.items(), key=operator.itemgetter(0), reverse=True)
	E_pl_s = sorted(E_pl.items(), key=operator.itemgetter(0), reverse=True)
	f_exp = open('data/exp_fastinf_edges.txt','w+')
	f_simp = open('data/powerlaw_fastinf_edges.txt','w+')
	f_pl = open('data/simple_fastinf_edges.txt', 'w+')
	f2_exp = open('data/exp_fastinf_loadedgelist.txt', 'w+')
	f2_pl = open('data/pl_fastinf_loadedgelist.txt', 'w+')
	f2_simp = open('data/simp_fastinf_loadedgelist.txt', 'w+')
...
def main():
	C = readfile('data/terror_cascade_1996-2016_ME_cascades.txt')
